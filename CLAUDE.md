...
> **Documents référencés dans un cadrage mais toujours absents de `docs/`** : seul `BRAND_BIBLE` reste non créé — décision explicite avec l'utilisateur (2026-08-03) de ne jamais le rédiger par extrapolation seule (choix de couleurs/logo/ton de voix), il nécessite un retour humain explicite pour ne pas violer la règle « aucune décision arbitraire ». `PRODUCT_BIBLE.md`, `FEATURE_BIBLE.md`, `UX_BIBLE.md` existent. `UX_GUIDELINES` est couvert par `docs/UX_PRINCIPLES.md`. `DESIGN_SYSTEM` reste `docs/DESIGN_SYSTEM_ARCHITECTURE.md` (technique, pas de marque). `MOTION_GUIDELINES.md` existe depuis la Phase 1 volume 3 (durées/courbes concrètes, au-delà des tokens de `docs/DESIGN_SYSTEM_ARCHITECTURE.md` §1). `INTERACTION_GUIDE` (nom proche de `INTERACTION_GUIDELINES.md`) n'a jamais été créé comme fichier séparé — le fichier existant a été étendu à chaque fois.
